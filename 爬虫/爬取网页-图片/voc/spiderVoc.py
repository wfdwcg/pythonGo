...
def getImg(imgInitlist,st):
    for imgInit in imgInitlist:
        try:
            ...
            if (imgInit.find("http://bbs.voc.com.cn/")) != 0:
                continue
            print("imgInit:" + imgInit)
            initHtml = getHtml(imgInit)
            reg = r'src="(.+?\.jpg)"'
            imgre = re.compile(reg)
            initHtml = initHtml.decode('gbk')  # python3
            imglist = imgre.findall(initHtml)
            x = 0
            for imgurl in imglist:
                #http://image.hnol.net/c/2015-03/24/17/201503241727275391-513051.jpg
                if (imgurl.find("http://image.hnol.net/")) != 0:
                    continue
                if (imgurl in st):
